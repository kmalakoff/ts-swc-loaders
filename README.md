...
20.19-22.14, plus Node's own unreliable `module.registerHooks()` window (22.15-22.22.2), get it from
a CommonJS `require()` hook this package installs; 22.22.3 and later, and 24/26, get it from the
same `module.registerHooks()` hooks, which serve both `import()` and `require()`. On Node
